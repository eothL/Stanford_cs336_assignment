...
- `local/`: User's personal gitignored scratch space. Do not write here unless User explicitly asks — agent implement-mode output goes to `Learning_hub/local/assignment_1/` instead.
...
### 3. Inference
- ⬜ Add a dedicated inference entrypoint in `Learning_hub/local/assignment_1/` (recommended: `generate.py`) that loads tokenizer + checkpoint + model config.
